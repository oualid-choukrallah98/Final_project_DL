...
class MambaDecoder(nn.Module):
    ...
    def __init__(
        ...
        try:
            ...
            tokenizer = AutoTokenizer.from_pretrained(vision_model_name)
            self.tokenizer = tokenizer
        except:
            # Fallback: use BLIP processor tokenizer
        ...
        if vocab_size == self.embedding.weight.shape[0]:
    ...
    def generate(self, pixel_values, max_new_tokens=50, temperature=1.0, top_k=50, top_p=0.9):
        ...
        for step in range(max_new_tokens):
            ...
            # Combine vision context with text embeddings
            if step == 0:
                # First step: use vision context
                x = torch.cat([vision_context, text_embeds], dim=1)
            else:
                # Subsequent steps: just use text embeddings
                x = text_embeds
